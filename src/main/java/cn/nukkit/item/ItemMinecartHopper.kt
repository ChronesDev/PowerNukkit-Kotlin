...
class ItemMinecartHopper @JvmOverloads constructor(meta: Integer? = 0, count: Int = 1) : Item(MINECART_WITH_HOPPER, meta, count, "Minecart with Hopper") {
    ...
    override fun onActivate(level: Level, player: Player, block: Block?, target: Block, face: BlockFace?, fx: Double, fy: Double, fz: Double): Boolean {
        if (Rail.isRailBlock(target)) {
            ...
            val minecart: EntityMinecartHopper = Entity.createEntity("MinecartHopper",
                    ...
                    .putList(ListTag("Rotation")
                            .add(FloatTag("", 0))
                            .add(FloatTag("", 0)))
            ) as EntityMinecartHopper ?: return false
