...
class ItemMinecartChest @JvmOverloads constructor(meta: Integer? = 0, count: Int = 1) : Item(MINECART_WITH_CHEST, meta, count, "Minecart with Chest") {
    ...
    override fun onActivate(level: Level, player: Player, block: Block?, target: Block, face: BlockFace?, fx: Double, fy: Double, fz: Double): Boolean {
        if (Rail.isRailBlock(target)) {
            ...
            val minecart: EntityMinecartChest = Entity.createEntity("MinecartChest",
                    ...
                    .putList(ListTag("Rotation")
                            .add(FloatTag("", 0))
                            .add(FloatTag("", 0)))
            ) as EntityMinecartChest ?: return false
