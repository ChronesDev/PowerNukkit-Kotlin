...
class ItemMinecartTNT @JvmOverloads constructor(meta: Integer? = 0, count: Int = 1) : Item(MINECART_WITH_TNT, meta, count, "Minecart with TNT") {
    ...
    override fun onActivate(level: Level, player: Player, block: Block?, target: Block, face: BlockFace?, fx: Double, fy: Double, fz: Double): Boolean {
        if (Rail.isRailBlock(target)) {
            ...
            val minecart: EntityMinecartTNT = Entity.createEntity("MinecartTnt",
                    ...
                    .putList(ListTag("Rotation")
                            .add(FloatTag("", 0))
                            .add(FloatTag("", 0)))
            ) as EntityMinecartTNT ?: return false
