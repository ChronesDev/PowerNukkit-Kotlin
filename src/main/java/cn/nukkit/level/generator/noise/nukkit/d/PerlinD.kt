 ...
 */
class PerlinD(random: NukkitRandom, octaves: Double, persistence: Double, expansion: Double) : NoiseD() {
    constructor(random: NukkitRandom, octaves: Double, persistence: Double) : this(random, octaves, persistence, 1.0) {}
    ...
    override fun getNoise3D(x: Double, y: Double, z: Double): Double {
        ...
        val AA1: Double = grad(this.perm.get(AA), x, y, z)
        val BA1: Double = grad(this.perm.get(BA), x - 1, y, z)
        val AB1: Double = grad(this.perm.get(AB), x, y - 1, z)
        val BB1: Double = grad(this.perm.get(BB), x - 1, y - 1, z)
        val AA2: Double = grad(this.perm.get(AA + 1), x, y, z - 1)
        val BA2: Double = grad(this.perm.get(BA + 1), x - 1, y, z - 1)
        val AB2: Double = grad(this.perm.get(AB + 1), x, y - 1, z - 1)
        val BB2: Double = grad(this.perm.get(BB + 1), x - 1, y - 1, z - 1)
    ...
    init {
        ...
        expansion = expansion
        this.offsetX = (random.nextFloat() * 256).toDouble()
        this.offsetY = (random.nextFloat() * 256).toDouble()
        this.offsetZ = (random.nextFloat() * 256).toDouble()
